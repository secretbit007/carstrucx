...
document.addEventListener('DOMContentLoaded', function() {
    var config = document.getElementById('script-config').dataset;
    var staticUrl = config.staticUrl;
    
    function loadScript(path) {
        var script = document.createElement('script');
        script.src = staticUrl + path;
        document.head.appendChild(script);
    }
    
    loadScript(config.easing);
    loadScript(config.cycle);
    loadScript(config.respond);
    
    if (document.querySelector('.video-image')) {
        loadScript(config.fancybox);
    }
});
/* end  */
